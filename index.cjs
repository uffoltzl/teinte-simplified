...
function interpolateColor(c1, c2, t) {
  ...
  while (maxChroma - minChroma > EPSILON) {
    ...
    // Step 14.4:
    else if (!isCurrentInRGBGamut) {
      // Step 14.4.1:
      let clippedCurrentLab = linearSrgbToOklab(clip(currentLinearRGB));

      // Step 14.4.2:
      let E = delta(clippedCurrentLab, currentLab);

      // Step 14.4.3:
      if (E < JUST_NOT_DISCERNABLE) {
        // Step 14.4.3.1:
        if (JUST_NOT_DISCERNABLE - E < EPSILON) {
          return scaleTo255(srgbLinearToGamma(oklabToLinearSrgb(clippedCurrentLab)));
        }
        // Step 14.4.3.2:
        else {
          // Step 14.4.3.2.1:
  ...
  return scaleTo255(srgbLinearToGamma(oklabToLinearSrgb(oklchToOklab({ ...cLch, c: chroma }))));
}
